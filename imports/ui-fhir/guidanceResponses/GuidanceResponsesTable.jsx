...
function GuidanceResponsesTable(props){
  logger.info('Rendering the GuidanceResponsesTable');
  ...
  // if(Array.isArray(guidanceResponses)){
    ...
    //   let dehydrateedCollection = guidanceResponses.map(function(record){
    //     return dehydrateGuidanceResponse(record, "YYYY-MM-DD");
    //   });      
    ...
    //     }
    //     if(get(row, 'guidanceResponseDisplay')){
    //       columnHasData.barcode = true;
    //     }
    //     if(get(row, 'guidanceResponse')){
    //       columnHasData.barcode = true;
    //     }
  ...
  function renderGuidanceResponseHeader(){
    if (!hideGuidanceResponse) {
      return (
        <TableCell className='guidanceResponse'>GuidanceResponse</TableCell>
      );
    }
  }
  function renderGuidanceResponse(guidanceResponseDisplay ){
    if (!hideGuidanceResponse) {
      return (
        <TableCell className='guidanceResponse'>{ guidanceResponseDisplay }</TableCell>
      );
    }
  } 
  ...
  function renderActionIcons( guidanceResponse ){
    if (!hideActionIcons) {
      ...
      return (
        <TableCell className='actionIcons' style={{width: '120px'}}>
          {/* <Icon icon={tag} style={iconStyle} onClick={showSecurityDialog.bind(this, guidanceResponse)} />
          <Icon icon={iosTrashOutline} style={iconStyle} onClick={removeRecord.bind(this, guidanceResponse._id)} /> */}
        </TableCell>
      );
    }
  } 
  ...
  function renderActionButton(guidanceResponseId){
    if (!hideActionButton) {
      return (
        <TableCell className='ActionButton' >
          <Button onClick={ handleActionButtonClick.bind(this, guidanceResponseId)}>{ get(props, "actionButtonLabel", "") }</Button>
        </TableCell>
      );
    }
  }
  ...
  function rowClick(id){
    // Session.set('selectedGuidanceResponseId', id);
    // Session.set('guidanceResponsePageTabIndex', 2);
  };
  ...
  if(guidanceResponses){
    if(guidanceResponses.length > 0){     
      let count = 0;    

      guidanceResponses.forEach(function(guidanceResponse){
        if((count >= (page * rowsPerPage)) && (count < (page + 1) * rowsPerPage)){
          guidanceResponsesToRender.push(FhirDehydrator.dehydrateGuidanceResponse(guidanceResponse, internalDateFormat));
        }
        count++;
      });  
    }
  ...
  if(guidanceResponsesToRender.length === 0){
    logger.trace('GuidanceResponsesTable: No guidanceResponses to render.');
  } else {
    for (var i = 0; i < guidanceResponsesToRender.length; i++) {
      let selected = false;
      if(guidanceResponsesToRender[i].id === selectedGuidanceResponseId){
        selected = true;
      }
      if(get(guidanceResponsesToRender[i], 'modifierExtension[0]')){
        rowStyle.color = "orange";
      }
      if(tableRowSize === "small"){
        rowStyle.height = '32px';
      }
      logger.trace('guidanceResponsesToRender[i]', guidanceResponsesToRender[i])

      if(get(guidanceResponsesToRender[i], "resourceType") === "OperationOutcome"){
        tableRows.push(
          <TableRow 
          className="immunizationRow" 
          key={i} 
          style={rowStyle} 
          onClick={ handleRowClick.bind(this, guidanceResponsesToRender[i].id)} 
          hover={true} 
          style={{height: '53px', background: "repeating-linear-gradient( 45deg, rgba(253,184,19, 0.9), rgba(253,184,19, 0.9) 10px, rgba(253,184,19, 0.75) 10px, rgba(253,184,19, 0.75) 20px ), url(http://s3-us-west-2.amazonaws.com/s.cdpn.io/3/old_map_@2X.png)"}} >            
            <TableCell className='actionIcons' style={{width: '100%', whiteSpace: 'nowrap'}}>
              {get(guidanceResponsesToRender[i], 'issue[0].text', 'OperationOutcome: No data returned.')}
            </TableCell>
        ...
        tableRows.push(
          <TableRow className="guidanceResponseRow" key={i} style={rowStyle} onClick={ handleRowClick.bind(this, guidanceResponsesToRender[i]._id)} style={rowStyle} hover={true} selected={selected} >            
            { renderCheckbox(i) }
            { renderActionIcons(guidanceResponsesToRender[i]) }
            { renderTextIcon(get(guidanceResponsesToRender[i], "text.div", "")) }
            { renderIdentifier(get(guidanceResponsesToRender[i], "identifier", "")) }

            { renderBarcode(get(guidanceResponsesToRender[i], "_id", ""))}
            { renderActionButton(get(guidanceResponsesToRender[i], "_id", "")) }
          </TableRow>
        );   
      }
  ...
  return(
    <div id={id} className="tableWithPagination">
      <Table className='guidanceResponsesTable' size={tableRowSize} aria-label="a dense table" { ...otherProps }>
        <TableHead>
          <TableRow>
...
GuidanceResponsesTable.propTypes = {
  id: PropTypes.string,
  data: PropTypes.array,
  guidanceResponses: PropTypes.array,
  selectedGuidanceResponseId: PropTypes.string,
...
GuidanceResponsesTable.defaultProps = {
  tableRowSize: 'medium',
  rowsPerPage: 5,
...
export default GuidanceResponsesTable;
