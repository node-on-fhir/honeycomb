...
function EvidencesTable(props){
  logger.info('Rendering the EvidencesTable');
  ...
  // if(Array.isArray(evidences)){
    ...
    //   let dehydrateedCollection = evidences.map(function(record){
    //     return dehydrateEvidence(record, "YYYY-MM-DD");
    //   });      
  ...
  function renderActionIcons( evidence ){
    if (!hideActionIcons) {
      ...
      return (
        <TableCell className='actionIcons' style={{width: '120px'}}>
          {/* <Icon icon={tag} style={iconStyle} onClick={showSecurityDialog.bind(this, evidence)} />
          <Icon icon={iosTrashOutline} style={iconStyle} onClick={removeRecord.bind(this, evidence._id)} /> */}
        </TableCell>
      );
    }
  } 
  ...
  function renderActionButton(evidenceId){
    if (!hideActionButton) {
      return (
        <TableCell className='ActionButton' >
          <Button onClick={ handleActionButtonClick.bind(this, evidenceId)}>{ get(props, "actionButtonLabel", "") }</Button>
        </TableCell>
      );
    }
  }
  ...
  function rowClick(id){
    // Session.set('selectedEvidenceId', id);
    // Session.set('evidencePageTabIndex', 2);
  };
    ...
    if(evidences.length > 0){     
      ...
      evidences.forEach(function(evidence){
        if((count >= (page * rowsPerPage)) && (count < (page + 1) * rowsPerPage)){
          evidencesToRender.push(FhirDehydrator.dehydrateEvidence(evidence, internalDateFormat));
        }
        count++;
      });  
    }
  ...
  if(evidencesToRender.length === 0){
    logger.trace('EvidencesTable: No evidences to render.');
  } else {
    for (var i = 0; i < evidencesToRender.length; i++) {
      let selected = false;
      if(evidencesToRender[i].id === selectedEvidenceId){
        selected = true;
      }
      if(get(evidencesToRender[i], 'modifierExtension[0]')){
        rowStyle.color = "orange";
      }
      if(tableRowSize === "small"){
        rowStyle.height = '32px';
      }
      logger.trace('evidencesToRender[i]', evidencesToRender[i])

      if(get(evidencesToRender[i], "resourceType") === "OperationOutcome"){
        tableRows.push(
          <TableRow 
          className="immunizationRow" 
          key={i} 
          style={rowStyle} 
          onClick={ handleRowClick.bind(this, evidencesToRender[i].id)} 
          hover={true} 
          style={{height: '53px', background: "repeating-linear-gradient( 45deg, rgba(253,184,19, 0.9), rgba(253,184,19, 0.9) 10px, rgba(253,184,19, 0.75) 10px, rgba(253,184,19, 0.75) 20px ), url(http://s3-us-west-2.amazonaws.com/s.cdpn.io/3/old_map_@2X.png)"}} >            
            <TableCell className='actionIcons' style={{width: '100%', whiteSpace: 'nowrap'}}>
              {get(evidencesToRender[i], 'issue[0].text', 'OperationOutcome: No data returned.')}
            </TableCell>
        ...
        tableRows.push(
          <TableRow className="evidenceRow" key={i} style={rowStyle} onClick={ handleRowClick.bind(this, evidencesToRender[i]._id)} style={rowStyle} hover={true} selected={selected} >            
            { renderCheckbox(i) }
            { renderActionIcons(evidencesToRender[i]) }
            { renderTextIcon(get(evidencesToRender[i], "text.div", "")) }
            { renderIdentifier(get(evidencesToRender[i], "identifier", "")) }
            { renderTitle(get(evidencesToRender[i], "title", "")) }
            { renderBarcode(get(evidencesToRender[i], "_id", ""))}
            { renderActionButton(get(evidencesToRender[i], "_id", "")) }
          </TableRow>
        );   
      }
  ...
  return(
    <div id={id} className="tableWithPagination">
      <Table className='evidencesTable' size={tableRowSize} aria-label="a dense table" { ...otherProps }>
        <TableHead>
          <TableRow>
...
EvidencesTable.propTypes = {
  id: PropTypes.string,
  data: PropTypes.array,
  evidences: PropTypes.array,
  selectedEvidenceId: PropTypes.string,
